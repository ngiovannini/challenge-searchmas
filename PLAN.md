...
- [x] T0.4 Configurar `serverless.yml` base + `serverless-offline`
...
- [x] T2.2 `getDataHandler.ts`: parsea query params, devuelve JSON + metadata

## Bloque 3 — Exportación CSV

- [x] T3.1 `ExportCsvUseCase`: reutiliza el `where` de T2.1 (sin paginar), convierte a CSV
- [x] T3.2 `exportCsvHandler.ts`: devuelve el archivo como descarga

## Bloque 4 — Tests

- [x] T4.1 Tests de `SyncDataUseCase` (mock de cliente HTTP y repositorio)
- [x] T4.2 Tests de `GetDataUseCase` (mock de repositorio)
- [x] T4.3 Tests de `ExportCsvUseCase`
